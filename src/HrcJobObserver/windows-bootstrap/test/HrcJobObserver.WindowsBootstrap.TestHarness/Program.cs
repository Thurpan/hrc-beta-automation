...
internal static partial class Program
{
    ...
    private static async Task<int> Main(string[] args)
    {
        ...
        TestCase[] tests =
        {
            ...
            new("trusted artifact set retains and revalidates every exact member", TestTrustedArtifactSetRoundTrip),
            new("trusted artifact set rejects incomplete and unexpected entries", TestTrustedArtifactSetEntryGuards),
            new("trusted artifact set rejects every member identity mismatch", TestTrustedArtifactSetIdentityMismatches),
            ...
            new("pinned release manifest retains its exact synthetic artifact set", TestPinnedReleaseManifestRoundTrip),
            new("pinned release manifest authenticates before canonical parsing", TestPinnedReleaseManifestAuthenticatesBeforeParse),
            new("pinned release manifest rejects noncanonical wires", TestPinnedReleaseManifestRejectsNoncanonicalWires),
            new("pinned release manifest binds the protected artifact-set digest", TestPinnedReleaseManifestBindsArtifactSetDigest),
            new("pinned release manifest enforces one absolute operation budget", TestPinnedReleaseManifestBounds),
            new("pinned release manifest has one fixed golden identity", TestPinnedReleaseManifestGoldenIdentity),
            new("native fixture PE audit owns one exact reproducible profile", TestNativeFixturePeAuditRoundTrip),
            new("native fixture PE audit rejects header section and directory drift", TestNativeFixturePeAuditImageLayout),
            new("native fixture PE audit rejects import and load-policy drift", TestNativeFixturePeAuditImportsAndLoadPolicy),
            new("native fixture PE audit rejects every debug identity drift", TestNativeFixturePeAuditDebugIdentity),
            new("native fixture PE audit rejects resource relocation and overlay drift", TestNativeFixturePeAuditResourcesAndOverlay),
            new("native fixture PE audit enforces checksum authentication and bounds", TestNativeFixturePeAuditAuthenticationAndBounds),
            new("native fixture roles run in a closed environment while Job containment remains deferred", TestNativeFixtureRuntimeRoles),
            new("native System32 module set retains the exact startup profile", TestNativeSystemModuleIdentityRoundTrip),
            new("native System32 module set owns matching load evidence", TestNativeSystemModuleIdentityEvidence),
            new("native System32 module set rejects mismatches and bounds work", TestNativeSystemModuleIdentityMismatchAndBounds),
            new("native launch-policy package owns one exact golden identity", TestNativeLaunchPolicyPackageRoundTripAndGoldenIdentity),
            new("native launch-policy package authenticates before canonical parsing", TestNativeLaunchPolicyPackageAuthenticatesBeforeParsing),
            new("native launch-policy package rejects noncanonical and mismatched nested policies", TestNativeLaunchPolicyPackageRejectsNoncanonicalAndMismatchedPolicies),
            new("native launch-policy package bounds authentication and rolls back failure", TestNativeLaunchPolicyPackageBoundsAndFailureRollback),
            new("native launch-policy file lease retains one exact outer-pinned package", TestNativeLaunchPolicyPackageFileRoundTrip),
            new("native launch-policy file lease authenticates before package parsing", TestNativeLaunchPolicyPackageFileAuthenticationOrder),
            new("native launch-policy file lease rejects unsafe namespace state", TestNativeLaunchPolicyPackageFileNamespaceGuards),
            new("native launch-policy file lease rolls back bounded failure", TestNativeLaunchPolicyPackageFileBoundsAndRollback),
            new("package-bound native Exit retains its selected outer-pinned authority", TestPackageBoundNativeFixtureContainmentExit),
            new("package-bound native launcher rejects incoherent policy before process creation", TestPackageBoundNativeFixtureContainmentPreCreateRejection),
            new("package-bound native launcher cleans every injected containment fault", TestPackageBoundNativeFixtureContainmentFaultCleanup),
            new("package-bound native launcher bounds selection launch and concurrent disposal", TestPackageBoundNativeFixtureContainmentBoundsAndDisposal),
            new("release manifest admits only closed legacy and native profiles", TestNativeReleaseManifestProfiles),
            new("release manifest preserves legacy and native golden identities", TestNativeReleaseManifestGoldenIdentity),
            new("retained artifact copies one bounded exact byte snapshot", TestRetainedArtifactExactByteCopy),
            new("retained artifact wipes late failed byte snapshots", TestRetainedArtifactCopyLateFailureCleanup),
            new("audited native release authenticates owns and revalidates its inputs", TestAuditedNativeReleaseAuthenticationAndRoundTrip),
            new("audited native release releases every partial failure", TestAuditedNativeReleaseFailureCleanup),
            new("audited native release wipes and releases late failures", TestAuditedNativeReleaseLateFailureOwnership),
            new("audited native containment enforces its platform and AMD64 debug ABI", TestAuditedNativeFixtureContainmentPlatform),
            new("audited native Exit launches by its exact retained image and exits zero", TestAuditedNativeFixtureContainmentExit),
            new("audited native Block retains its application-directory pin through explicit Job close", TestAuditedNativeFixtureContainmentAncestorPin),
            new("audited native containment cleans every injected launch-stage fault", TestAuditedNativeFixtureContainmentFaultCleanup),
            new("audited native containment rejects late success and coalesces disposal", TestAuditedNativeFixtureContainmentDeadlineAndDisposal),
